...
namespace HerosInsight::SkillBook
{
    ...
    using SkillProp = SkillFiltering::SkillProp;
    using AttributeSource = SkillFiltering::AttributeSource;
    ...
    public:
        ...
        void DrawTooltip(uint32_t tooltip_id)
        {
            ...
            for (size_t i = 0; i < cell_count; ++i)
            {
                ...
                {
                    ImGuiCustom::TextFont font_scope(style.font);
    ...
    class SkillFracProvider : public RichText::TextFracProvider
    {
        ...
        void DrawFraction(ImVec2 pos, FracTag tag) override
        {
            ...
            ImGuiCustom::TextSize font_scope2(-2);
            auto num_pos = pos;
        ...
        float CalcWidth(FracTag tag) override
        {
            ...
            ImGuiCustom::TextSize font_size_scope(-2);
    ...
    struct BookState
    {
        ...
        void DrawDupeButton()
        {
            ImGuiCustom::TextFont font_scope(Constants::Fonts::window_name_font); // Why??
            auto window = ImGui::GetCurrentWindow();
            ...
            if (ImGuiCustom::RadioArray("Attributes:", (int *)&attr_src.type, options, IM_ARRAYSIZE(options)))
            {
                ...
                switch (attr_src.type)
                {
                    ...
                    case AttributeSource::Type::Manual:
                        attr_src.value = settings.attr_rank_slider;
                        break;
                }
            }

            if (attr_src.type == AttributeSource::Type::Manual)
            {
        ...
        void InitFilterList()
        {
            ...
            for (auto skill_id_16 : SkillFiltering::base_skills)
            {
                ...
                {
                    auto focused_agent_data = CustomAgentDataModule::GetCustomAgentData(focused_agent_id);
                    if (focused_agent_data.GetOrEstimateAttribute(cskill.attribute) == 0)
                        continue;
                }
        ...
        void DrawProperty(DrawContent &&draw_content, size_t prop_id, bool is_hidden_header = false)
        {
            ...
            auto DrawTooltip = [](std::string_view text, std::span<uint16_t> highlighting)
            {
                ...
                {
                    ImGuiCustom::TextFont font_scope{};
        ...
        void DrawSkillHeader(CustomSkillData &custom_sd, bool is_equipable)
        {
            ...
            { // Draw stuff rightward of icon
                ...
                { // Draw skill name
                    ImGuiCustom::TextFont font_scope(Constants::Fonts::skill_name_font);
        ...
        bool Draw(IDirect3DDevice9 *device, size_t book_index)
        {
            ...
            bool is_open = true;
            if (ImGuiCustom::WindowScope hi_wnd{name.data(), &is_open})
            {
                DrawDupeButton();
    ...
    void DrawHelpContent(BookState *book)
    {
        ...
        auto DrawExample = [&](std::string_view query_body, std::string_view description)
        {
            if (book)
            {
                ...
                buffer.push_back('\0');
                if (ImGuiCustom::Button(buffer.data()))
                {
    ...
    void Draw(IDirect3DDevice9 *device)
    {
        ...
        {
            bool is_open = true;
            if (ImGuiCustom::WindowScope hi_wnd{"Search-syntax help", &is_open})
            {
